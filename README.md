...
The bank structure supports conversion to QTI packages for use with Canvas. This fork has them disabled by default to reduce build size for Cloudflare Pages. Enable by setting "include_qti: bool = True" in the build settings.

## ▶ Exam Builder (web app)

Deployment method to be determined.

A single, self-contained page — no install required — for browsing the problem banks, assembling an exam, previewing the rendered math, and exporting to Canvas QTI or printable HTML. The problem banks are embedded directly in the page; an internet connection is only needed for math rendering (KaTeX). The page is generated by [`scripts/build_standalone_html.py`](./scripts/build_standalone_html.py) and published from [`docs/standalone.html`](./docs/standalone.html) via Cloudflare Pages.
